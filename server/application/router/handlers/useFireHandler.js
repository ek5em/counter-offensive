const useFireHandler = (answer, userManager, gameManager) => {
        ...
        if(token && x!==false && y!==false && angle!==false){
            if (!isNaN(angle) && !isNaN(x) && !isNaN(y)) {
                ...
                if (user[0] && user[0].token != null) {
                    const result = await gameManager.fire(user[0].id, parseFloat(x), parseFloat(y), parseFloat(angle));
                    if (isNaN(parseInt(result))) res.json(answer.good(result)); 
                    else res.json(answer.bad(result));   
                }
                else res.json(answer.bad(401));
            }
            else res.json(answer.bad(422));
        }
        else res.json(answer.bad(400));
    }
};

module.exports = useFireHandler;